...
void CityDataTranslate() {
	ifstream read_in;
	read_in.open("att48.txt");        // 待转换数据，数据可从上述链接【城市数据】下载
	if (!read_in.is_open())
	{
		cout << "文件读取失败." << endl;
		return;
	...
	ofstream fout("\\city_48.txt");        // 转换后的数据存入文档 city_150.txt
...
void TSP_Evaluate(Graph G) {
	...
	for (int i = 0;i < GROUP_NUM; i++)
	{
		...
		for (int j = 0; j < G.vex_num; j++)
		{
			cout<<TSP_Groups[i].path[j]<<" -> ";
		}
		cout<<TSP_Groups[i].path[0]<<"    ";
		cout<<"length = "<<TSP_Groups[i].length_path<<"    P_Reproduction = "<<TSP_Groups[i].P_Reproduction<<endl;
	}
	*/
